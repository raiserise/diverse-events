...
const {
  ...
  describe("updateRSVP", () => {
    ...
    test("should update RSVP successfully to approved by organizer", async () => {
      ...
      db.runTransaction.mockImplementation((updateFn) => updateFn(fakeTransaction));
    ...
    test("should throw error due to cancellation cooldown", async () => {
      ...
      await expect(updateRSVP(rsvpId, userId, newStatus)).rejects.toThrow(/You must wait/);
    });
  });
  ...
  describe("getRSVPsByEvent", () => {
    ...
    test("should throw error if fetching RSVPs for event fails", async () => {
      ...
      await expect(getRSVPsByEvent(eventId)).rejects.toThrow(/Error fetching RSVPs for event: Fetch error/);
    });
  });
  ...
  describe("findRSVP", () => {
    test("should return RSVP if found", async () => {
      ...
      const getMock = jest.fn().mockResolvedValue({ empty: false, docs: fakeDocs });
    ...
    test("should throw error if lookup fails", async () => {
      ...
      await expect(findRSVP(eventId, userId)).rejects.toThrow(/RSVP lookup failed: Lookup error/);
    });
  });
  ...
  describe("getUserRSVPs", () => {
    test("should return RSVPs for the given user", async () => {
      ...
      const fakeDocs = [
        { id: "rsvp1", data: () => ({ userId, status: "pending", createdAt: fakeTimestamp }) },
        { id: "rsvp2", data: () => ({ userId, status: "approved", createdAt: fakeTimestamp }) },
      ];
      const getMock = jest.fn().mockResolvedValue({ docs: fakeDocs, empty: false });
      const whereMock = jest.fn().mockReturnValue({ get: getMock });
      db.collection.mockImplementation(() => ({ where: whereMock }));
    ...
    test("should throw error if fetching user RSVPs fails", async () => {
      ...
      await expect(getUserRSVPs(userId)).rejects.toThrow(/Error fetching user RSVPs: User fetch error/);
    });
  });
