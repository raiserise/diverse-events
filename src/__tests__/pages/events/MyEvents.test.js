...
jest.mock('../../../components/EventModal', () => ({ isOpen }) =>
...
describe('MyEvents component', () => {
  ...
  it('displays event cards after successful fetch and transformation', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('event-card')).toHaveTextContent('React Workshop');
    });
  });
  ...
  it('filters events by title and format', async () => {
    jest.spyOn(useUserEventsHook, 'useUserEvents').mockReturnValue({
      events: [
        { id: '1', title: 'React Workshop', format: 'Online', startDate: '2025-05-01', endDate: '2025-05-01' },
        { id: '2', title: 'Vue Meetup', format: 'Offline', startDate: '2025-05-01', endDate: '2025-05-01' },
      ],
      loading: false,
      error: null,
    });
